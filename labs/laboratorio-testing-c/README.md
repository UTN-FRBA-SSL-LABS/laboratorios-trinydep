...
> R: Lo que hace `carrito_init` es inicializar el carrito y establecer su cantidad de productos en 0. Es importante llamarla antes de usar el carrito ya que si no se inicializa, el campo cantidad podria contener valores basura y el comportamiento de las funciones que trabajan con el carrito seria indefinido.
...
> R: Este test falla. Esperaba el valor 700 y obtuvo el valor 350.
...
> R: El bug está en la línea 24:  `total += c->items[i].precio;`. El código suma el precio de los productos, pero no tiene en cuenta la cantidad de cada producto. 

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Esa línea debería multiplicar el precio del producto por su cantidad y sumar ese resultado al total.
...
> R: Si, las lineas 29 y 30 de `carrito.c` no se ejecutaron. Son las lineas correspondientes a la funcion `carrito_descuento` y no se ejecutaron ya que los tests unitarios no llaman ni utilizan esa funcion. 
...
> R: Un test unitario es aquel que prueba una funcion funciona correctamente de manera aislada, mientras que un test de integracion verifica que las funciones trabajen bien en combinacion. El bug de `carrito_total` fue detectado primero por el test unitario `test_total_con_cantidad`.
